...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    const timeoutId = setTimeout(() => {
      setLoading(false);
      setError('Google Sign-In request timed out. Check your API keys and configuration.');
    }, 10000);
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message && error.message.includes('not enabled')) {
          setError('Google Auth is not enabled! Enable "Google" in your Supabase Auth Providers settings.');
        } else {
      ...
      clearTimeout(timeoutId);
      setLoading(false);
      setError('Failed to connect to authentication server. Check your Supabase URL/Anon Key.');
    }
  };
