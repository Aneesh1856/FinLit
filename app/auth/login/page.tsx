  ...
  const handleGoogle = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        // ───────────────────────────────────────────────────────
        // NATIVE ANDROID: Use @capacitor/browser (in-app browser)
        // This is the ONLY reliable way to do OAuth in Capacitor.
        // ───────────────────────────────────────────────────────
        const { Browser } = await import('@capacitor/browser');
        const { App } = await import('@capacitor/app');

        // 1. Get the OAuth URL from Supabase without triggering redirect
        const { data, error } = await supabase.auth.signInWithOAuth({
          provider: 'google',
          options: {
            redirectTo: 'com.finlit.ai://login-callback',
            skipBrowserRedirect: true,  // <-- KEY: Don't redirect, just give us the URL
        ...
        // 2. Open the auth URL in an in-app browser
        ...
        // ───────────────────────────────────────────────────────
        // WEB: Standard redirect flow (this already works)
        // ───────────────────────────────────────────────────────
